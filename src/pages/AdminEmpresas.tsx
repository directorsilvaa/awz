...
const EmpresasManager: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userData = {
        ...
        numero_processo: userForm.numero_processo,
      ...
      if (editingUser) {
        // Edit user
        await axios.put(
          `http://localhost:8000/api/auth/empresas/${editingUser._id}`,
          userData,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
        ...
        // Create new user
        await axios.post(
          "http://localhost:8000/api/auth/empresas/register",
          userData,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `http://localhost:8000/api/auth/empresas/${userToDelete._id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  return (
    <div className="py-12 px-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Gestão de Empresas</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">
          Empresas Cadastrados ({filteredUsers?.length})
        </h2>
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4 max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold text-gray-800">
                {editingUser ? "Editar Empresa" : "Nova Empresa"}
              </h2>
              <button
                onClick={closeModal}
            ...
            <div onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  value={userForm.numero_processo}
                  onChange={(e) =>
                    setUserForm({
                      ...userForm,
